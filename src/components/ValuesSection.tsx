...
const ValuesSection = () => {
  ...
  return <section id="valores" ref={sectionRef} className="section-padding bg-background relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          ...
          <div className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-12"}`}>
            ...
            <div className="mb-12">
              ...
              <p className="text-lg text-muted-foreground leading-relaxed pl-[74px]">Prestar serviços de excelência em Field Service, Outsourcing e Enterprise Solutions, trazendo valor acrescentado aos nossos clientes. Oferecer soluções de importação e exportação com qualidade, responsabilidade e respeito ao meio ambiente, garantindo a satisfação dos clientes e a valorização dos nossos colaboradores.</p>
            </div>
          ...
          <div className={`transition-all duration-1000 delay-300 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-12"}`}>
            <span className="inline-block px-4 py-2 bg-secondary/10 text-secondary rounded-full text-sm font-semibold mb-6">Nossos valores</span>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {values.map((value, index) => <div key={index} className="p-5 rounded-xl bg-card border border-border/50 hover:shadow-md transition-all duration-300 group hover:border-secondary/30" style={{
